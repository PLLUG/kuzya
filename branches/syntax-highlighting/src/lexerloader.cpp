...
QFont LexerLoader::defaultFont(int style) const
{
    ...
    if((style==wordStyleDomElement.attribute("styleID","").toInt()))
    {

        if(fontDataBase.families().contains(wordStyleDomElement.attribute("fontName","no")))
        {
            f=QFont(wordStyleDomElement.attribute("fontName","Monospace"));
        }
        else
        {
            f=QFont(styleNode.toElement().attribute("defaultFontName","Monospace"));
            f.setPointSize(styleNode.toElement().attribute("defaultFontSize","9").toInt());
        }
        if (""!=wordStyleDomElement.attribute("fontSize",styleNode.toElement().attribute("defaultFontSize","9")))
        {
            f.setPointSize(wordStyleDomElement.attribute("fontSize",styleNode.toElement().attribute("defaultFontSize","9")).toInt());
        }
        else
        {
             f.setPointSize(styleNode.toElement().attribute("defaultFontSize","9").toInt());
        ...
        if(0==wordStyleDomElement.attribute("fontStyle","").toInt()) {f.setBold(false); f.setItalic(false);}
        ...
        if(3==wordStyleDomElement.attribute("fontStyle","").toInt()) {f.setBold(true); f.setItalic(true);}
...
{
    QColor paperColor = QsciLexer::defaultPaper(style);;
    QDomNode styleNode = domNode.namedItem("LexerStyle");
    QDomElement wordStyleDomElement = styleNode.firstChildElement();

    while((!wordStyleDomElement.isNull()&&(style!=wordStyleDomElement.attribute("styleID","-1").toInt())))
    {
        if("DEFAULT"==wordStyleDomElement.attribute("name",""))
        {
            paperColor = QColor(wordStyleDomElement.attribute("bgColor","#FFFFFF"));
        }
        wordStyleDomElement=wordStyleDomElement.nextSiblingElement();
        //qDebug()<<wordStyleDomElement.attribute("name","");
    }
    ...
    if(style==wordStyleDomElement.attribute("styleID","-1").toInt())
    {
        return QColor(wordStyleDomElement.attribute("bgColor",paperColor.name()));
    }
    else
    {
        return paperColor;
    ...
    /*
        ...
        while((!wordStyleDomElement.isNull()&&("DEFAULT"!=wordStyleDomElement.attribute("name",""))))
        {
            wordStyleDomElement=wordStyleDomElement.nextSiblingElement();
            //qDebug()<<wordStyleDomElement.attribute("name","");
        }
        ...
        else return QsciLexer::defaultPaper(style);*/
